...
function output_zone_constraint(square: Square, grid: Square[][]): PathNode | undefined {
    ...
    for (let yDelta = 1; yDelta <= zoneSize && (y + yDelta) < grid.length; yDelta++) {
        ...
        if (squareToCheck.type == 'S' && (squareToCheck.direction == 'N' || squareToCheck.direction == 'S')) { return {x: squareToCheck.x, y: squareToCheck.y, is_skip: false} }
    }

    // Check north
    for (let yDelta = 1; yDelta <= zoneSize && (y - yDelta) >= 0; yDelta++) {
        const squareToCheck = grid[y - yDelta][x]
        if (squareToCheck.type == 'S' && (squareToCheck.direction == 'S' || squareToCheck.direction == 'N')) { return {x: squareToCheck.x, y: squareToCheck.y, is_skip: false} }
    }

    // Check east
    for (let xDelta = 1; xDelta <= zoneSize && (x + xDelta) < grid[y].length; xDelta++) {
        const squareToCheck = grid[y][x + xDelta]
        if (squareToCheck.type == 'S' && squareToCheck.direction == 'W') { return {x: squareToCheck.x, y: squareToCheck.y, is_skip: false} }
    }

    // Check west
    for (let xDelta = 1; xDelta <= zoneSize && (x - xDelta) >= 0; xDelta++) {
        const squareToCheck = grid[y][x - xDelta]
        if (squareToCheck.type == 'S' && squareToCheck.direction == 'E') { return {x: squareToCheck.x, y: squareToCheck.y, is_skip: false} }
    }
...
function search_node_from_square(square: Square, traversable: boolean, splitter_node_coords?: PathNode, is_finish=false, is_start=false): SearchNode {
    return {
        x: square.x,
        y: square.y,
        traversable,
...
export function connection_between_squares_on_grid(first: Square, second: Square, squares: Square[][]): PathNode[] {
    let grid: SearchNode[][] = build_search_grid_from_squares(squares, first, second)
    let path: PathNode[] = shortest_path_between_nodes(search_node_from_square(first, false, undefined, true), search_node_from_square(second, true), grid)
